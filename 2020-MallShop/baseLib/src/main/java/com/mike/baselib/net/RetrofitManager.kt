...
//    }
    ...
    private val RewriteCacheControlInterceptor = object : Interceptor {
        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            val url = request.url().toString() //获取请求URL
            val buffer = Buffer()
            requestBody.writeTo(buffer)
            val params = buffer.readString(Charset.forName("UTF-8")) //获取请求参数
            ...
            if (NetworkUtil.isNetworkAvailable(AppContext.getInstance().context)) {
                ...
                val maxAge =onlineCacheTime
                //如果网络正常，执行请求。
                val originalResponse = chain.proceed(request)
                //获取MediaType，用于重新构建ResponseBody

                val originalBody=originalResponse.body()!!
                logTools.d(originalBody.contentType())
                val type = originalResponse.body()!!.contentType()
                //获取body字节即响应，用于存入数据库和重新构建ResponseBody
                val bs = originalResponse.body()!!.bytes()
                response = originalResponse.newBuilder()
                        ...
                        .build()
                //将响应插入数据库
               // cacheDao.insertResponse(url, params, String(bs, "GB2312"))
                AppBusManager.insertResponseCache(url,params,String(bs))
                val cache= AppBusManager.queryResponseCache(url,params)
                logTools.d(cache)
                if(cache!=null){
                    logTools.d(cache.url)
                    logTools.d(cache.pramas)
                }
                val list= LitePal.findAll(ResponseCache::class.java)
                if(list!=null){
                    for(i in list){
                ...
                logTools.d(offlineCacheTimeStr)
                logTools.d(offlineCacheTimeStr != null && offlineCacheTimeStr.ext_isPureNumber_orDecimal())
                if (offlineCacheTimeStr != null && offlineCacheTimeStr.ext_isPureNumber_orDecimal()) {
                    offlineCacheTime = offlineCacheTimeStr.toInt()
                }else{
                    return chain.proceed(request)
                }
                val b = cache.body //读出响应
                logTools.d("request:$url")
                logTools.d("request method:" + request.method())
                val maxStale = offlineCacheTime
                //构建一个新的response响应结果
                response = Response.Builder()
                        .removeHeader("Pragma")
                        .header("Cache-Control", "public, only-if-cached, max-stale=$maxStale")
                       // .body(ResponseBody.create(MediaType.parse("application/json"), b.toByteArray()))
                        .body(ResponseBody.create(MediaType.parse("application/json;charset=UTF-8"), b))
                        .request(request)
    ...
    class EncryptInterceptor : Interceptor {
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            logTools.d(response.header("Date"))
            logTools.d(Date())
            //logTools.d(Date)
    ...
    private fun getOkHttpClient(): OkHttpClient {
        ...
        //设置 请求的缓存的大小跟位置
        val cacheFile = File(AppContext.getInstance().context.cacheDir, "okhttpCache")
...
//              .addInterceptor(addCacheInterceptor())
                ...
                .addInterceptor(RewriteCacheControlInterceptor) //POST 缓存
