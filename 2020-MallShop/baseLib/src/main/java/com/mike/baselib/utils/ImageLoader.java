...
public class ImageLoader {
     ...
     *加载图片(默认)
     */
    public static void loadImage(Context context, String url,int holderRes, ImageView imageView) {
        RequestOptions options = new RequestOptions()
                .placeholder(holderRes) //占位图
                .error(holderRes)       //错误图
     ...
     * 加载圆形图片
     */
    public static void loadCircleImage(Context context, String url, int holderRes,int errorRes,ImageView imageView) {
        RequestOptions options = new RequestOptions()
                .centerCrop()
                .circleCrop()//设置圆形
     ...
     * 加载圆角图片
     */
    public static void loadRoundCornersImage(Context context, String url,int radius,int holderRes, ImageView imageView) {
        BitmapTransformation t=new CenterCrop();
        ...
        RequestOptions options=new RequestOptions()
                .transforms(t, new RoundedCorners(radius)
                ) .placeholder(holderRes)
                .error(holderRes).diskCacheStrategy(DiskCacheStrategy.ALL);
        Glide.with(context)
                .load(url)
                .apply(options)
                .into(imageView);
     ...
     * 加载圆角图片
     */
    public static void loadRoundCornersImage(Context context, int res,int radius, ImageView imageView) {
        RequestOptions options=new RequestOptions()
                .transforms(new CenterCrop(), new RoundedCorners(radius)
                ) .placeholder(placeholderSoWhite)
                .error(errorSoWhite) .diskCacheStrategy(DiskCacheStrategy.ALL);
        Glide.with(context)
                .load(res)
                .apply(options)
                .into(imageView);
    ...
    public static void loadCustRoundCircleImage(Context context, String url, ImageView imageView, RoundedCornersTransformation.CornerType type) {
        RequestOptions options =  RequestOptions .bitmapTransform(new RoundedCornersTransformation(45, 0, type))
                .centerCrop()
                .placeholder(placeholderSoWhite)
                .error(errorSoWhite)
                //.priority(Priority.HIGH)
...
//                .diskCacheStrategy(DiskCacheStrategy.ALL);
       RequestOptions options= RequestOptions.bitmapTransform(new GrayscaleTransformation())
                .centerCrop()
                .placeholder(placeholderSoWhite)
                .error(errorSoWhite)
                //.priority(Priority.HIGH)
    ...
    public static void loadGif(Context context, int resId,int holderRes, ImageView imageView) {
        RequestOptions options = new RequestOptions()
                .placeholder(holderRes)
                .error(holderRes);
        Glide.with(context).asGif()
