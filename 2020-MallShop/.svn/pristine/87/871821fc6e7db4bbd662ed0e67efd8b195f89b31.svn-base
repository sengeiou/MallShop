...
class SearchGoodsListAdapter(mContext: Context, list: ArrayList<SearchGoods>, layoutId: Int = R.layout.item_search_goodslist) :
        CommonAdapter<SearchGoods>(mContext, list, layoutId) {

    interface OnItemClickListener {
        fun onClick(item: SearchGoods)
    }
    ...
    override fun bindData(holder: ViewHolder, data: SearchGoods, position: Int) {
        holder.getView<TextView>(R.id.tvDesc).text=data.shoppingMallName
        val tvDesc=holder.getView<TextView>(R.id.tvDesc)
        //tvDesc.ext_Highlight(data.shoppingMallName,keyword)
        val ivImage=holder.getView<ImageView>(R.id.ivImage)
        ivImage.ext_loadConnersImageWithDomain(data.picUrl)
