...
import com.epro.mall.utils.zbar.BarcodeFormat;
...
public class ZBarView extends QRCodeView {
    ...
    private LogTools logTools=new LogTools(this.getClass().getSimpleName()+"_");
    ...
    public void setType(BarcodeType barcodeType, List<BarcodeFormat> formatList) {
        ...
        if (mBarcodeType == BarcodeType.CUSTOM && (mFormatList == null || mFormatList.isEmpty())) {
            throw new RuntimeException(AppContext.getInstance().getString(R.string.zbar_formatlist_null));
        }
        setupReader();
    }
    ...
    protected ScanResult processData(byte[] data, int width, int height, boolean isRetry) {
        ...
        if(scanBoxAreaRect!=null){
            logTools.d(width);
            logTools.d(height);
            logTools.d(mScanBoxView.getX());
            logTools.d(mScanBoxView.getY());
            logTools.d(mScanBoxView.getWidth());
            logTools.d(mScanBoxView.getHeight());
            logTools.d(mScanBoxView.getRectWidth());
            logTools.d(mScanBoxView.getRectHeight());
            logTools.d(scanBoxAreaRect.left);

            logTools.d(scanBoxAreaRect.width());
            logTools.d(scanBoxAreaRect.height());
        }
