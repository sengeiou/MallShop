...
class UpdateAppActivity : BaseTitleBarCustomActivity<UpdateAppContract.View, UpdateAppPresenter>(), UpdateAppContract.View, View.OnClickListener {
    ...
    var beanArrayList = ArrayList<String>()   //首页强制更新内容
    var aboutArrayList = ArrayList<String>() //关于app更新内容
    //版本号获取成功
    ...
    override fun onAboutSuccess(result: AboutBean.Result) {
        ...
        if (updateType == UPDATE_TYPE_PAGE) { //页面
            ...
            if (doUpdate) {
                ...
                var mList = updateInfo!!.content.split("；")  //多条更新用分号隔开
                for (element in mList) {
                    aboutArrayList.add(element)
                }
                rvListView.layoutManager = LinearLayoutManager(this)
                rvListView.adapter = UpdateContentAdapter(this,aboutArrayList)
            } else {
