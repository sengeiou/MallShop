...
public class EmojiDao {
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        //File dir = new File("data/data/" + "com.epro.comp.im" + "/databases");
        LogTools.debug(TAG, AppContext.getInstance().getContext().getPackageName());
        File dir = new File("data/data/" + AppContext.getInstance().getContext().getPackageName() + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                LogTools.debug(TAG, AppContext.getInstance().getContext());
                inputStream = AppContext.getInstance().getContext().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
                ...
                e.printStackTrace();
                LogTools.getInstance().t(TAG).e(e);
                LogTools.debug(TAG,e.getMessage());
                LogTools.debug(TAG, AppContext.getInstance().getContext());
