...
public class ChatAdapter extends BaseQuickAdapter<ChatMessage, BaseViewHolder> {
    ...
    private void setStatus(BaseViewHolder helper, ChatMessage item) {
        ...
        private void setContent(BaseViewHolder helper, ChatMessage item) {
            CustomerService customerService = IMBusManager.Companion.getCustomerService(item.getCsId());
            String avatarPath = IMBusManager.Companion.getUser().getAvatar();
            String basePath = AppConfig.Companion.getBaseurl()+"/image/";
                if (item.getMsgType().equals(MsgType.TEXT)){
                   ...
                   ImageView avatar = helper.getView(R.id.chat_item_header);
                   if (!item.isSend()){
                       ImageLoader.loadCircleImage(mContext, basePath+customerService.getAvatar(),R.mipmap.ic_head_default_left,R.mipmap.ic_head_default_left,avatar);
                       helper.setText(R.id.serviceName,customerService.getName());
                   }else {
                       ImageLoader.loadCircleImage(mContext,basePath+avatarPath ,R.mipmap.ic_head_default_right,R.mipmap.ic_head_default_right,avatar);
                   }
